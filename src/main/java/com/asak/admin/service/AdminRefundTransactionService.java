...
public class AdminRefundTransactionService {
  ...
  public void applyRefund(
      RefundTarget target, String refundReason, String providerCancelTransactionKey) {

    Long approvedStatusId =
        adminPaymentMethodMapper.findPaymentMethodStatusId(PaymentStatus.APPROVED.name());

    Long refundedStatusId =
        adminPaymentMethodMapper.findPaymentMethodStatusId(PaymentStatus.REFUNDED.name());

    if (approvedStatusId == null || refundedStatusId == null) {
      throw new CustomException(ErrorCode.PAYMENT_METHOD_STATUS_NOT_FOUND);
    }
    ...
    refundMap.put("providerCancelTransactionKey", providerCancelTransactionKey);
    ...
    if (OrderStatus.COMPLETED.name().equals(target.getOrderStatus())) {
      return;
    }

    // 제공 전 환불이면 CANCELED
    Long canceledStatusId = adminOrderMapper.findOrderStatusId(OrderStatus.CANCELED.name());
