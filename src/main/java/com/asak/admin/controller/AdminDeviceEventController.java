...
/**
 * 관리자 재출력/로그 조회와, 예제 형식의 RTOS polling 결과 보고를 임시로 함께 둔다. RTOS 인증 정책이 정해지면 RTOS
 * endpoint는 별도
...
public class AdminDeviceEventController {
  ...
  public ApiResponse<DeviceEventResponse> requestReceiptPrint(
    ...
    DeviceEventResponse response = deviceEventService.createReceiptPrintEvent(orderId, request, "ADMIN");
  ...
  public ApiResponse<DeviceEventResponse> claimNextPendingEvent() {
    return deviceEventService
        ...
        .map(
            response -> ApiResponse.success("RTOS_DEVICE_EVENT_CLAIMED", "처리할 장치 이벤트입니다.", response))
  ...
  public ApiResponse<DeviceEventResponse> finishEvent(
    ...
    DeviceEventResponse response = deviceEventService.finishEvent(eventId, request.status(), request.result());
