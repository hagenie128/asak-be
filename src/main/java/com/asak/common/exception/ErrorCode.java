...
/** API business error catalog. The code is a stable client contract. */
public enum ErrorCode {
  INVALID_OPTION_SELECTION("INVALID_OPTION_SELECTION", HttpStatus.BAD_REQUEST, "유효하지 않은 옵션 선택입니다."),
  OPTION_ITEM_SOLD_OUT("OPTION_ITEM_SOLD_OUT", HttpStatus.CONFLICT, "선택한 옵션 항목이 품절되었습니다."),
  ITEM_QUANTITY_LIMIT_EXCEEDED(
      "ITEM_QUANTITY_LIMIT_EXCEEDED", HttpStatus.BAD_REQUEST, "메뉴 수량 제한을 초과했습니다."),
  CART_QUANTITY_LIMIT_EXCEEDED(
      "CART_QUANTITY_LIMIT_EXCEEDED", HttpStatus.BAD_REQUEST, "장바구니 전체 수량 제한을 초과했습니다."),
  CART_EMPTY("CART_EMPTY", HttpStatus.BAD_REQUEST, "장바구니가 비어 있습니다."),
  ...
  INVALID_ORDER_REQUEST("INVALID_ORDER_REQUEST", HttpStatus.BAD_REQUEST, "올바르지 않은 주문 요청입니다."),
  INVALID_ORDER_TYPE("INVALID_ORDER_TYPE", HttpStatus.BAD_REQUEST, "올바른 주문 유형이 아닙니다."),
  ORDER_INITIAL_STATUS_NOT_FOUND(
      "ORDER_INITIAL_STATUS_NOT_FOUND", HttpStatus.INTERNAL_SERVER_ERROR, "주문 초기 상태를 찾을 수 없습니다."),
  ...
  ORDER_DAILY_SEQUENCE_EXCEEDED(
      "ORDER_DAILY_SEQUENCE_EXCEEDED", HttpStatus.INTERNAL_SERVER_ERROR, "일일 주문번호 생성 시도를 초과했습니다."),
  ORDER_CREATE_FAILED("ORDER_CREATE_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "주문을 생성할 수 없습니다."),
  ORDER_ITEM_CREATE_FAILED(
      "ORDER_ITEM_CREATE_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "주문 항목을 생성할 수 없습니다."),
  ORDER_OPTION_CREATE_FAILED(
      "ORDER_OPTION_CREATE_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "주문 옵션을 생성할 수 없습니다."),
  ORDER_EXCLUSION_CREATE_FAILED(
      "ORDER_EXCLUSION_CREATE_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "제외 재료를 생성할 수 없습니다."),
  ...
  ORDER_STATUS_CONFLICT("ORDER_STATUS_CONFLICT", HttpStatus.CONFLICT, "주문 상태가 이미 변경되었습니다."),
  ...
  INVALID_ORDER_QUERY("INVALID_ORDER_QUERY", HttpStatus.BAD_REQUEST, "잘못된 주문 조회 조건입니다."),
  ORDER_AMOUNT_MISMATCH("ORDER_AMOUNT_MISMATCH", HttpStatus.CONFLICT, "주문 금액이 일치하지 않습니다."),

  PAYMENT_CREATE_FAILED("PAYMENT_CREATE_FAILED", HttpStatus.CONFLICT, "결제 생성에 실패했습니다."),
  PAYMENT_METHOD_DISABLED("PAYMENT_METHOD_DISABLED", HttpStatus.CONFLICT, "결제 수단이 비활성화되었습니다."),
  ...
  IDEMPOTENCY_KEY_CONFLICT("IDEMPOTENCY_KEY_CONFLICT", HttpStatus.CONFLICT, "다른 요청에 사용된 멱등성 키입니다."),
  PAYMENT_DUPLICATE("PAYMENT_DUPLICATE", HttpStatus.CONFLICT, "이미 처리 중인 결제입니다."),

  CATEGORY_NOT_FOUND("CATEGORY_NOT_FOUND", HttpStatus.NOT_FOUND, "카테고리를 찾을 수 없습니다."),
  MENU_IMAGE_SAVE_FAILED(
      "MENU_IMAGE_SAVE_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "메뉴 이미지를 저장할 수 없습니다."),
  ...
  INGREDIENT_NOT_FOUND("INGREDIENT_NOT_FOUND", HttpStatus.NOT_FOUND, "재료를 찾을 수 없습니다."),
  ...
  MENU_UPDATE_NOT_FOUND("MENU_UPDATE_NOT_FOUND", HttpStatus.NOT_FOUND, "수정할 메뉴를 찾을 수 없습니다."),
  ...
  MENU_DELETE_NOT_FOUND("MENU_DELETE_NOT_FOUND", HttpStatus.NOT_FOUND, "삭제할 메뉴를 찾을 수 없습니다."),
