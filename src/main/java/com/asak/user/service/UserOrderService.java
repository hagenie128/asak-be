...
public class UserOrderService {
  ...
  private ValidatedOrderResult validateAndPriceItems(
    ...
    for (OrderItemCommand item : requestedItems) {
      ...
      // null이면 빈 목록으로 정규화
      List<OptionItemRequest> requestedOptions =
          item.getOptionItems() == null ? List.of() : item.getOptionItems();
      ...
      for (OptionItemRequest option : requestedOptions) {
        ...
        if (Boolean.TRUE.equals(optionInfo.getIsSoldOut())) {
      ...
      // 4. 옵션 정책 검증
      List<OptionPolicyQueryDto> policies =
          orderMapper.findOptionPoliciesByMenuId(item.getMenuId());
      ...
      for (OptionPolicyQueryDto policy : policies) {
        ...
        int requiredMinSelect =
            Boolean.TRUE.equals(policy.getIsRequired()) ? Math.max(1, minSelect) : minSelect;
      ...
      List<Long> excludedIngredientIds =
          item.getExcludedIngredientIds() == null ? List.of() : item.getExcludedIngredientIds();

      for (Long ingredientId : excludedIngredientIds) {

        Long validIngredientId =
            orderMapper.findRemovableIngredient(item.getMenuId(), ingredientId);
    ...
    return new ValidatedOrderResult(totalAmount, validatedItems);
  }
  ...
  public CartValidateResponse cartValidate(CartValidateRequest request) {
    ...
    for (int i = 0; i < result.items().size(); i++) {
      responseItems.add(
          toCartValidateItemResponse(
              request.getItems().get(i).getClientCartItemId(), result.items().get(i)));
    }
  ...
  private CartValidateItemResponse toCartValidateItemResponse(
      String clientCartItemId, ValidatedOrderItem item) {
    ...
    response.setExcludedIngredientIds(item.excludedIngredientIds());

    List<CartValidateOptionItemResponse> options =
        item.optionItems().stream()
            .map(
                option -> {
                  CartValidateOptionItemResponse optionResponse =
                      new CartValidateOptionItemResponse();

                  optionResponse.setOptionItemId(option.optionItemId());
                  optionResponse.setQuantity(option.quantity());
  ...
  /** 주문/장바구니 공통 수량 제한 검증 각 장바구니 아이템은 1~9개, 모든 아이템 수량의 합계는 30개까지 허용 */
  private <T> void validateItemQuantityLimits(
      List<T> items, Function<T, Integer> quantityExtractor) {
   ...
   * ③ common_code에서 orderType에 해당하는 orderTypeId와 READY 상태의 statusId를 조회한다.
   ...
   * ⑨ orderId, orderNo, totalAmount, READY만 CreateOrderResponse에 담아 반환한다.
   *
  ...
  public CreateOrderResponse createOrder(CreateOrderRequest request) {
    ...
    OrderStatus initialStatus = OrderStatus.READY;
    ...
    // 주문 상태 확인
    Long statusId = orderMapper.findOrderStatusId(initialStatus.name());

    if (statusId == null) {
      throw new CustomException(ErrorCode.ORDER_INITIAL_STATUS_NOT_FOUND);
    }
    ...
    if (insertedOrderCount != 1 || order.getOrderId() == null) {

      throw new CustomException(ErrorCode.ORDER_CREATE_FAILED);
    }
    ...
    for (ValidatedOrderItem item : result.items()) {
      ...
      int insertedItemCount = orderMapper.insertOrderItem(orderItem);
      // orderItem이 제대로 저장되었는지 유무 확인 로직
      if (insertedItemCount != 1 || orderItem.getOrderItemId() == null) {

        throw new CustomException(ErrorCode.ORDER_ITEM_CREATE_FAILED);
      }
      ...
      for (ValidatedOptionItem option : item.optionItems()) {
        ...
        int insertedOptionCount =
            orderMapper.insertOrderItemOption(
                orderItemId, option.optionItemId(), option.quantity(), option.extraPrice());

        // 옵션 아이템 저장 실패시
        if (insertedOptionCount != 1) {
          throw new CustomException(ErrorCode.ORDER_OPTION_CREATE_FAILED);
        }
      }
      ...
      for (Long ingredientId : item.excludedIngredientIds()) {
        ...
        int insertedExclusionCount = orderMapper.insertItemExclusion(orderItemId, ingredientId);

        if (insertedExclusionCount != 1) {
          throw new CustomException(ErrorCode.ORDER_EXCLUSION_CREATE_FAILED);
        }
      }
    }
    ...
    // OrderStatus enum은 JSON 직렬화 시 초기 주문 상태인 "READY" 문자열로 반환된다.
  ...
  private String generateOrderNo() {
    ...
    String lastOrderNo = orderMapper.findLastOrderNoByPrefix(orderNoPrefix);
    ...
    if (lastOrderNo != null) {
      ...
      if (lastOrderNo.length() != orderNoPrefix.length() + 4
        ...
        throw new CustomException(ErrorCode.ORDER_NUMBER_CREATE_FAILED);
      }

      try {
        // 5. 마지막 주문번호의 뒤 4자리를 숫자로 변환하고 1을 더한다.
        String lastSequencePart = lastOrderNo.substring(orderNoPrefix.length());
    ...
    if (nextSequence > MAX_DAILY_ORDER_SEQUENCE) {
      throw new CustomException(ErrorCode.ORDER_DAILY_SEQUENCE_EXCEEDED);
    }

    // 6. 다음 순번을 4자리로 채우고 최종 주문번호를 반환한다.
    return orderNoPrefix + String.format("%04d", nextSequence);
  }
