...
토큰이 다르게 나와도 의미까지 다른 것은 아니다. MySQL 은 조인 트리에 중첩 괄호를
붙여 저장하는데 `view.sql` 은 가독성을 위해 그 괄호를 뺐다. 그래서 `verify` 는
표기가 다른 뷰에 대해 **실제 뷰와 문서 SQL 을 각각 실행해** 행 수, 전체 행 체크섬
(`BIT_XOR(CRC32)` + `SUM(CRC32)`), 컬럼 이름·순서를 비교한다. 셋 다 같으면 표기만
다른 것으로 판정한다.
