...
CREATE TABLE `orders` (
    ...
    `waiting_order_no` INT DEFAULT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `order_no` (`order_no`),
    UNIQUE KEY `uq_waiting_no_per_day` (`waiting_date`,`waiting_order_no`),
...
CREATE TABLE `payment` (
    ...
    `provider_payment_key` VARCHAR(200) DEFAULT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_payment_idempotency_key` (`idempotency_key`),
    UNIQUE KEY `uk_payment_provider_payment_key` (`provider_payment_key`),
...
CREATE TABLE `payment_refund` (
    ...
    `provider_cancel_transaction_key` VARCHAR(200) DEFAULT NULL,
    `refunded_at` TIMESTAMP NULL DEFAULT NULL,
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_payment_refund_provider_cancel_transaction_key` (`provider_cancel_transaction_key`),
    KEY `idx_payment_refund_payment_id` (`payment_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
ALTER TABLE `payment_refund` ADD CONSTRAINT `fk_payment_refund_payment`
